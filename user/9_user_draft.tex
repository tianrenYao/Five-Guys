...
HQ managers operate at the corporate level, serving as a critical bridge between daily operational oversight and organisation-wide governance management. They conduct cross-store performance reviews, oversee end-to-end ESG governance frameworks, configure operational and compliance thresholds, evaluate business-wide regulatory adherence, and administer centralised organisational user records. Given that the majority of advanced administrative and management modules are exclusively tailored to their responsibilities, their role requires prominent emphasis throughout the user manual.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{the Alerts page.png}
    \caption{the Alerts page7.2}
    \label{fig:alerts-page}
  \end{figure}
  \begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{the Add Threshold modal.png}
    \caption{the Add Threshold modal 7.2}
    \label{fig:add-threshold-modal}
  \end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{supplier ESG table with grades visible.png}
    \caption{supplier ESG table with grades visible 7.3}
    \label{fig:supplier-esg-table}
  \end{figure}
  \begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{add and edit supplier modal showing scorecard.png}
    \caption{add and edit supplier modal showing scorecard 7.3}
    \label{fig:supplier-scorecard-modal}
  \end{figure}


\subsection*{7.4 Company Policy Management}
The policy tab functions as a lightweight internal document management page (rather than a reporting page), supporting the creation, editing, review, and deletion of ESG-related internal policies. Managers can set the policy’s title, category, status, effective date, version, and content. Additionally, policies can be viewed in read mode and edited in form mode.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{the policy list.png}
    \caption{the policy list 7.4}
    \label{fig:policy-list}
  \end{figure}
  

\subsection*{7.5 User Management}
The User Management page is accessible to HQ managers and admins, featuring summary cards, a user table, an add-user modal, edit functionality, password reset, and user deletion options. Additionally, this page supports role assignment and scope assignment, which can be configured through the region and store fields.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{main user table with summary cards.png}
    \caption{main user table with summary cards 7.5}
    \label{fig:user-table-summary}
  \end{figure}
  \begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{add-user modal.png}
    \caption{add-user modal 7.5}
    \label{fig:add-user-modal}
  \end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{the Audit Log page.png}
    \caption{the Audit Log page 7.6}
...
Admins are the system’s highest-level users. Their user-facing workflow closely overlaps with HQ managers, but they have broader authority and fewer organisational restrictions. Thus, the user manual may describe admins as an extended management role instead of dedicating a separate, lengthy chapter.
...
\subsection*{9.1 How to Insert and Store Images}
For consistency, store all screenshots in the \texttt{images} folder under the same directory as this \texttt{.tex} file. Then insert images by file name using \texttt{\textbackslash includegraphics}:
...
In conclusion, the platform supports various user groups via role-based workflows and offers a practical browser-based environment for sustainability operations and ESG management. This user manual focuses on implemented functions rather than theoretical future features.
